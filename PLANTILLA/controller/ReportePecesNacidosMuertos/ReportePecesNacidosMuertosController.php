...
class ReportePecesNacidosMuertosController
{
    ...
    public function exportarPecesExcel()
    {
        ...
        $sheet->setCellValue('A10', 'PECES NACIDOS Y MUERTOS POR TANQUE');
        $sheet->getStyle('A10')->getFont()->setBold(true)->setSize(11)->getColor()->setRGB($azulOscuro);
        $sheet->getStyle('A10')->getBorders()->getBottom()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM)->getColor()->setRGB($azulOscuro);
        ...
        if (empty($datos)) {
            ...
            $sheet->getStyle("A$fila:E$fila")->getBorders()->getTop()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM)->getColor()->setRGB($azulOscuro);
            $sheet->getStyle("A$fila:E$fila")->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        }
        ...
        // ================== GRÁFICA ESTADÍSTICA ==================
        if (!empty($datos)) {
            $categorias = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, "'Peces por Tanque'!\$A\${$primeraFilaDatos}:\$A\${$ultimaFilaDatos}", null, count($datos));
            $valoresNacidos = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_NUMBER, "'Peces por Tanque'!\$C\${$primeraFilaDatos}:\$C\${$ultimaFilaDatos}", null, count($datos));
            $valoresMuertosM = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_NUMBER, "'Peces por Tanque'!\$D\${$primeraFilaDatos}:\$D\${$ultimaFilaDatos}", null, count($datos));
            $valoresMuertosH = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_NUMBER, "'Peces por Tanque'!\$E\${$primeraFilaDatos}:\$E\${$ultimaFilaDatos}", null, count($datos));

            $etiquetaNacidos = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, "'Peces por Tanque'!\$C\${$filaEncabezado}", null, 1);
            $etiquetaMuertosM = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, "'Peces por Tanque'!\$D\${$filaEncabezado}", null, 1);
            $etiquetaMuertosH = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, "'Peces por Tanque'!\$E\${$filaEncabezado}", null, 1);
            ...
            $titulo = new Title('Peces nacidos y muertos por tanque');
