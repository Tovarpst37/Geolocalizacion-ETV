...
include_once '../model/ReportesTipoDeposito/ReportesTipoDepositoModel.php'; // ajusta a la ruta real de tu modelo
...
class ReportesTipoDepositoController
{
    ...
    public function getReporteTipoDeposito()
    {
        ...
        if ($generar) {
            ...
            $tiposSeleccionado = $_GET['tipo_deposito'] ??[];

            if (empty($tiposSeleccionado)) {
                include_once '../model/Errores/ErrorModal.php';

                ErrorModal::verError(
                    ["Seleccione al menos un tipo un tipo de deposito antes de generar el reporte."],
            ...
            $sql = "SELECT td.id_tipo_deposito, td.nombre AS tipo_deposito,
                ...
                AND sd.id_estado = 1
                WHERE td.id_tipo_deposito = ANY($1)
                GROUP BY td.id_tipo_deposito, td.nombre
                ORDER BY td.nombre";
            $datosReporte= $obj->select($sql, [$tiposSeleccionado]);
        ...
        include_once '../view/ReportesTipoDeposito/TipoDeposito.php';
    }
    ...
    public function exportarTipoDepositoExcel()
    {
        ...
        if (!is_array($tiposSeleccionado)){
            $tiposSeleccionado = [$tiposSeleccionado];
        }
        ...
        if (empty($tiposSeleccionado)) {
            ...
            ErrorModal::verError(
                ["Seleccione al menos un tipo de deposito antes descargar el reporte."],
                getUrl('ReportesTipoDeposito', 'ReportesTipoDeposito', 'getReporteTipoDeposito')
            );
            return;
        }

          $tiposSeleccionado = array_map('intval', $tiposSeleccionado);
        ...
        $sheet->setCellValue('A1', 'REPORTE DE SITOS POR TIPO DE DEPOSITO');
        ...
        $sheet->setCellValue('A4', 'TIPOS DE DEPOSITO');
        ...
        $sheet->setCellValue('A7', 'Tipos de deposito seleccionados:');
        $sheet->setCellValue('B7', count($datosReporte));

        $sheet->setCellValue('A8', 'Fecha de generacion:');
        ...
        $sheet->setCellValue('A11', 'SITIOS POR TIPO DE DEPOSITO');
        ...
        $filaEncabezado = 13;
        $sheet->setCellValue("A$filaEncabezado", 'ID tipo de deposito');
        $sheet->setCellValue("B$filaEncabezado", 'Tipo de deposito');
         $sheet->setCellValue("B$filaEncabezado", 'Cantida de sitios');
        ...
        foreach ($datosReporte as $t) {
            ...
            $sheet->setCellValue("B$fila", $t['tipo_deposito']);
             $sheet->setCellValue("B$fila", $t['cantidad']);
        ...
        if (empty($datosReporte)) {
            ...
            $sheet->setCellValue("A$fila", 'No se encontraron sitios de deposito para los tipos seleccionados .');
        ...
        $ultimaFilaDatos = $fila - 1;
            $categorias = new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING, "'Tipo de Deposito'!\$A\${$primeraFilaDatos}:\$A\${$ultimaFilaDatos}", null, count($datosReporte));
            ...
            $titulo = new Title('Cantidad de sitios por tipo de deposito');
